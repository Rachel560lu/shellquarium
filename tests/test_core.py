...
def test_tick_can_kill_pet_from_neglect():
    pet = Pet(name="Pixel", stage=LifeStage.BABY, hunger=0, health=2, poop=3, pomodoro_streak=5)
    events = pet.tick(now=pet.last_tick_at + 1)
    assert pet.stage != LifeStage.DEAD
    assert pet.pomodoro_streak == 0
    assert any("Neglect broke your focus streak" in event for event in events)
...
def test_dead_pet_shows_hatch_new_pet_controls():
    async def run() -> None:
        app = TerminalPetApp()
        app.pet = Pet(name="Pixel", stage=LifeStage.ELDER)
        async with app.run_test() as pilot:
            await pilot.pause()
            panel = app.query_one("#new-pet-panel", Static)
            assert "active pet" in str(panel.renderable).lower()
...
def test_streak_chest_awards_collection_drop(monkeypatch):
    app = TerminalPetApp()
    app.pet = Pet(name="Pixel", stage=LifeStage.BABY, pomodoro_streak=3)
    app._focus_index = 0
    app.FOCUS_DURATION_SECONDS = 1
    app.BREAK_DURATION_SECONDS = 2

    monkeypatch.setattr("shellquarium.ui.app.random.random", lambda: 0.0)
    monkeypatch.setattr("shellquarium.ui.app.random.choice", lambda choices: "Coral Kid")

    app.action_toggle_pomodoro()
    events = app._update_pomodoro((app._pomodoro_end_at or 0) + 0.1)

    assert app.pet.pomodoro_streak == 4
    assert app.pet.streak_chests_opened == 1
    assert app.pet.collection_unseen is True
    assert app.pet.character_shards.get("Coral Kid") == 1
    assert any("Streak Chest opened" in event for event in events)
    assert any("Collection updated" in event for event in events)


def test_collection_view_clears_unseen(monkeypatch, tmp_path):
    monkeypatch.setenv("SHELLQUARIUM_HOME", str(tmp_path))
    app = TerminalPetApp()
    app.pet = Pet(name="Pixel", stage=LifeStage.BABY, collection_unseen=True)
    app._focus_index = 2

    app.action_toggle_collection()

    assert app._shell_panel_mode == "collection"
    assert app.pet.collection_unseen is False


def test_pomodoro_scene_hides_crab_and_shell_and_shows_clock():
    pet = Pet(name="Pixel", stage=LifeStage.BABY)
    scene = render_scene_with_reaction(
        "tank",
        pet,
        0,
        scene_mode="pomodoro",
